...
8. **Letter e-signature (NativeSign hookup)** — shipped: a "Sign" action on any generated **PDF** letter in the Letters card sends it to the client through the existing in-app NativeSign flow (`backend/esign.py`) — the client signs via magic link, the signed PDF is filed in the `Signed` folder and logged. Closes the disbursement-acknowledgment e-sign gap (the disbursement letter's client signature block is now actionable). Uses NativeSign (free, self-hosted); **DocuSign remains a deferred v2 stub** (`docusign_bridge.py`) — NativeSign is primary by design, and no paid signing provider is required.  
