...
## 2. CURRENT STATE (honest, as of 2026-07-11)
...
- **Test suite exists but was not verified passing in this session.** 19 test files under `backend/tests/` cover every `pi_*` module (`test_pi_meds.py`, `test_pi_demand.py`, `test_pi_letters.py`, etc.) plus `test_praxium_api.py`, `test_outgoing_webhooks.py`, `test_training.py`. Running `pytest` in this environment fails to collect — first with `ModuleNotFoundError: No module named 'server'` (path issue), then with `KeyError: 'MONGO_URL'` once `PYTHONPATH=.` was set (no local Mongo / `.env` configured in this session). This is an environment-setup gap, not evidence the code is broken, but it is **unverified**, not "green."
...
- **Wave-2 systems (Citation OS, 8 practice-area modules, Booking-vs-WellSky spec, Billing OS, VA phase-out plan) are not present anywhere in `Main`.** A repo-wide grep for `citation os`, `billing os`, `va phase`, `wellsky`, `practice-area` found no matching content on `Main` beyond incidental mentions (e.g., a "dog-bite = adjacent practice area" backlog line in `IMPROVEMENTS.md`, a `practice_area` matter field). These exist only on `claude/praxium-expansion-systems` (PR #2, stacked on PR #1) per prior-session context — unverified in this pass.
...
- **Billing is a manual stub**, not the "Billing OS" concept from planning. `backend/billing.py` (116 lines) exposes three static plans (Starter $299/Growth $599/Enterprise custom) and an `UpgradeInquiry` model — no live Stripe charge flow, no metered marketplace lead billing, no trust/IOLTA accounting yet. This is the platform's own subscription billing, distinct from a firm-facing time/invoice "Billing OS" (planned, not built).
...
1. Two unmerged branches (PR #1, PR #2) carrying real product + legal content that is not in `Main` and is drifting further from it the longer it stays unmerged.
